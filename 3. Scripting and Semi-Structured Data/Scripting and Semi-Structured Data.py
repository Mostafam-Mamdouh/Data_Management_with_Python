...
# Description: A script that deal with Semi-Structured Data (json files),
#              perform simple ETL, read json files from source directory,
#              transform it, and then load it as csv into target directory.
#              The script takes 2 positional arguments (src_dir, dest_dir), and
#              1 optional arguments (-u/--unix_time) to maintain the UNIX format of timpe stamp 
...
def get_unique_json(src_dir):
    ...
    try:
        files = [item for item in os.listdir(src_dir) if os.path.isfile(os.path.join('.', item))]
    ...
    for filename in files:
        ...
        if(os.path.splitext(filename)[1] == ".json"):
            ...
            proc = Popen(['sha256sum', filename], stdout=PIPE, stderr=PIPE)
